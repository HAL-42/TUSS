...
def work(pkl_idx, _, cfg_pkl, cfg_rslt_dir):
    ...
    # * 根据分到的配置，运行任务。
    seg_metric_file = Path(cfg_rslt_dir) / 'cls' / 'eval' / 'seg' / 'statistics.pkl'
    if (not args.purge) and seg_metric_file.is_file():
        ...
        # * 在当前设备上执行训练。
        subprocess.run([sys.executable, 'src/tasks/gather_clip_by_samq_classify/run.py',
                        '-b', str(args.benchmark),
                        '-d', str(args.is_debug),
                        '-c', cfg_pkl],
...
def gather_metric(_: Config, cfg_rslt_dir: str, __: ..., ___: dict[str, tuple[..., str]]) \
        -> dict[str, ...]:
    return read_seg_cls_metric(Path(cfg_rslt_dir) / 'cls' / 'eval')
