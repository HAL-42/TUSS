...
__all__ = ['COCO_NAMES', 'COCO_COLOR', 'COCO91to81', 'COCO81to91', 'COCO2014']
...
class COCO2014(Dataset):
    ...
    def __init__(self, root: str="datasets", year="2014", split: str="train", subsplit: str="",
                 label_type: str='',
                 ...
                 PIL_read: bool=True,
                 cocostuff_img: bool=False,
                 cocostuff_lb: bool=False):
        # * 记录路径参数。
        self.root = root
        self.year = year
        ...
        self.cocostuff_img = cocostuff_img
        ...
        # * 图像级标签选择。
        assert cls_labels_type in ('online', 'seg_cls_labels', None)
        ...
        # * 是否使用COCOStuff标签。
        self.cocostuff_lb = cocostuff_lb
        ...
        # * 读取图像级标签。
        if self.cls_labels_type == 'online':
            self.id2cls_labels = 'online'
        elif self.cls_labels_type is not None:
    ...
    def _set_files(self):
        ...
        self.image_dir = osp.join(self.root, 'images_fine' if self.cocostuff_img else 'images',
                                  f'{self.split}{self.year}')
        if self.cocostuff_lb:
            self.label_dir = osp.join(self.root, 'third_party', 'masks')
        else:
    ...
    def get_item(self, index: int) -> Dict:
        ...
        if self.id2cls_labels == 'online':
            out.cls_lb = lb2cls_lb(lb, self.class_num, self.ignore_label)
        elif self.cls_labels_type is not None:
