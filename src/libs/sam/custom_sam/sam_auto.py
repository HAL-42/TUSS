...
from segment_anything.utils.amg import (MaskData, calculate_stability_score, batched_mask_to_box,
                                        ...
                                        box_xyxy_to_xywh, area_from_rle, generate_crop_boxes)
...
class SamAnns(list):
    ...
    def clear_data(self):
        ...
        # -* 清理缓存属性。
        for key in [k for k in vars(self).keys() if k != 'data']:
            del self.__dict__[key]

    @property
    def stacked(self) -> bool:
        return 'device' in self.data
    ...
    def stack_data(self, masks: bool=True, areas: bool=True, device=torch.device('cpu'), replace: bool=False):
        """将data在纵向维度stack。
        ...
        Args:
            ...
            replace: 若为True，顺手将ann中的segmentation替换为mask。
        ...
        if masks:
            m = [torch.as_tensor(SamAuto.decode_segmentation(ann, replace=replace), dtype=torch.bool, device=device)
                 for ann in self]
    ...
    def list_key(self, key: str) -> list:
        """返回所有标注的key值。
        ...
        Returns:
            键值列表。
        """
        return [ann[key] for ann in self]

    @cached_property
    def masks(self) -> torch.Tensor:
        m = [torch.as_tensor(SamAuto.decode_segmentation(ann, replace=False), dtype=torch.bool)
             for ann in self]
        return torch.stack(m, dim=0)

    @cached_property
    def cuda_masks(self) -> torch.Tensor:
        m = [torch.as_tensor(SamAuto.decode_segmentation(ann, replace=False), dtype=torch.bool, device='cuda')
             for ann in self]
        return torch.stack(m, dim=0)

    @cached_property
    def img_area(self) -> int:
        return self[0]['img_hw'][0] * self[0]['img_hw'][1]

    @cached_property
    def areas(self) -> torch.Tensor:
        return torch.as_tensor(self.list_key('area'), dtype=torch.long)

    @cached_property
    def cuda_areas(self) -> torch.Tensor:
        return torch.as_tensor(self.list_key('area'), dtype=torch.long, device='cuda')

    @cached_property
    def cuda_bbox_areas(self) -> torch.Tensor:
        xywh = torch.as_tensor(self.list_key('bbox'), dtype=torch.long, device='cuda')
        bbox_areas = xywh[:, 2] * xywh[:, 3]
        return bbox_areas

    @cached_property
    def levels(self) -> torch.Tensor:
        return torch.as_tensor(self.list_key('level'), dtype=torch.long)

    @cached_property
    def cuda_levels(self) -> torch.Tensor:
        return torch.as_tensor(self.list_key('level'), dtype=torch.long, device='cuda')
...
class SamAuto(SamAutomaticMaskGenerator):
    ...
    def _generate_masks(self, image: np.ndarray) -> MaskData:
        ...
        for crop_box, layer_idx in zip(crop_boxes, layer_idxs):
            ...
            data.cat(crop_data, no_deepcopy=True)  # CHANGE 因为crop_data是临时变量，所以不需要深拷贝。
    ...
    def _process_crop(
        ...
        for (points,) in batch_iterator(self.points_per_batch, points_for_image):
            ...
            data.cat(batch_data, no_deepcopy=True)  # 将每个batch的结果拼接起来。CHANGE: 因为batch_data是临时变量，所以不需要深拷贝。
            del batch_data
    ...
    def rock_sand_water(self, mask_data: MaskData) -> MaskData:
        ...
        for level_mask_data in levels_mask_datas:
            ...
            level_mask_data.filter(level_filter)
            rsw_mask_data.cat(level_mask_data, no_deepcopy=True)  # level_mask_data是临时变量，不需要深拷贝。
